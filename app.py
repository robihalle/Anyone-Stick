...
import subprocess, time, os, signal, re, socket
...
# ═══════════════════════════════════════════════════════════════════════
#  AnonController – talks to the anon daemon via ControlPort 9051
# ═══════════════════════════════════════════════════════════════════════

class AnonController:
    """Communicates with the anon daemon via its Tor-compatible control port."""
    ...
    def _connect(self):
        """Open connection and authenticate (cookie -> empty password -> bare)."""
    ...
    def _cmd(self, sock, command):
        """Send a single command on an existing connection."""
        sock.sendall(f"{command}\r\n".encode())
        return self._recv(sock)

    def command(self, cmd):
        """One-shot: connect -> auth -> command -> quit."""
        try:
            sock = self._connect()
            if not sock:
                return None
    ...
    def get_bootstrap(self):
        """Return dict with progress (0-100), summary, tag."""
    ...
    def get_status(self):
        """Return overall connection state dict."""
    ...
    def get_circuit_detail(self):
        """Return the first BUILT general-purpose circuit with IP + country."""
        try:
            sock = self._connect()
            if not sock:
                return None

            # 1) get circuit-status
            raw = self._cmd(sock, "GETINFO circuit-status")
            if not raw:
                self._cmd(sock, "QUIT"); sock.close()
                return None

            # find first BUILT circuit (prefer PURPOSE=GENERAL)
            built_path = None
            ...
            # assign roles based on hop count
            if len(hops) == 1:
                roles = ["exit"]
            elif len(hops) == 2:
                roles = ["entry", "exit"]
            ...
            # 3) enrich each hop with IP + country (on same connection)
...
/* ── Connection status polling (every 5s) ───────── */
...
/* ── Circuit chain polling (every 8s) ───────────── */
...
pollStatus();  pollCircuit();  pollTraffic();
